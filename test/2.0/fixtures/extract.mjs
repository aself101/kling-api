...
const REQUEST_FIXTURES = [
  ...
  { out: 'requests/omni-kling-3.0-omni-prompt.json', doc: 'kling-omni-3.0-ovg.md', section: '## Create Task', request: true },
  { out: 'requests/omni-kling-3.0-omni-first-refer.json', doc: 'kling-omni-3.0-ovg.md', section: '### First_frame & refer_image', request: true },
  { out: 'requests/omni-kling-3.0-omni-frames-elements.json', doc: 'kling-omni-3.0-ovg.md', section: '### First_frame & last_frame & element', request: true },
  { out: 'requests/omni-kling-3.0-omni-feature-video.json', doc: 'kling-omni-3.0-ovg.md', section: '### feature_video', request: true },
  { out: 'requests/omni-kling-3.0-omni-base-video.json', doc: 'kling-omni-3.0-ovg.md', section: '### base_video', request: true },
  { out: 'requests/omni-kling-o1-first-refer.json', doc: 'kling-o1-video.md', section: '## Create Task', request: true },
  { out: 'requests/omni-kling-o1-element-feature-video.json', doc: 'kling-o1-video.md', section: '### First_frame & multi-image_elements & feature_video', request: true },
...
function extractRequestBody(doc, section) {
  ...
  // A "### <scenario>" section under "More Scenario Examples" holds its bash fence directly.
  const anchor = section.startsWith('### ') ? start : lines.findIndex((l, i) => i > start && l.trim() === '### Request Example');
  if (anchor < 0) throw new Error(`${doc}: no Request Example after ${section}`);
  // The main examples are fenced ```bash; the scenario examples ```cURL.
  const open = lines.findIndex((l, i) => i > anchor && /^```(bash|curl)$/i.test(l.trim()));
