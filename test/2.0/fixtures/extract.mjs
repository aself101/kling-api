...
const index = ['# Codec fixtures — provenance', '',
  'Generated by `node test/2.0/fixtures/extract.mjs` from `docs/api/` (vendor snapshot, spec D17). Do not hand-edit the JSON; edit the `subs` table in the script and regenerate. Columns: fence line is the 1-based line of the ```json fence in the source page; each substitution is `path: vendor placeholder → fixture value`.', '',
  '| Fixture | Source page | Section | Fence line | Substitutions |', '|---|---|---|---:|---|'];
const written = new Set();
for (const f of FIXTURES) {
  const { text, line } = extractBlock(f.doc, f.section);
  let json;
  ...
  written.add(f.out);
  index.push(`| \`${f.out}\` | \`docs/api/${f.doc}\` | ${f.section.replace(/^#+\s*/, '')} | ${line} | ${subs.join('<br>') || '—'} |`);
}
