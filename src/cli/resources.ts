...
interface ElementCreateOptions extends CreateFlagOptions {
  name: string;
  description: string;
...
interface ElementDeleteOptions {
  kind: 'video' | 'image';
}
interface VoiceCreateOptions extends CreateFlagOptions {
  name: string;
  voiceUrl?: string;
  videoId?: string;
}
interface AvatarOptions extends CreateFlagOptions {
  image: string;
  audioId?: string;
  soundFile?: string;
  prompt?: string;
...
function printTasks(g: GlobalOptions, tasks: Task[]): void {
  const rows = tasks.flatMap((t) =>
    t.outputs.map((o) => ({
      taskId: t.id,
      status: t.status,
      ...(o.type === 'element' || o.type === 'voice'
        ? {
            id: o.id,
            name: o.name,
            ownedBy: 'ownedBy' in o ? o.ownedBy : undefined,
            itemStatus: o.status,
          }
        : { type: o.type }),
    }))
...
const page = (cmd: Command) =>
  cmd
    .option('--page <n>', 'page number, 1–1000', int('pageNum'))
...
export function registerResources(program: Command): void {
  const elements = program
    .command('elements')
    .description('element library (characters / subjects for video and image)');
  elements
    .command('create')
    ...
    .option(
      '--refer-image <file|url>',
      'additional angle/close-up, repeatable 1–3 (image_refer)',
      collect
    )
    ...
    .action(
      handler<ElementCreateOptions>(async (o, cmd) => {
        ...
        const handle = await client.elements.create({
          ...
          frontalImage: o.frontalImage ? mediaArg(o.frontalImage) : undefined,
          referImages: o.referImage?.map(mediaArg),
          referVideos: o.referVideo ? [o.referVideo] : undefined,
          voiceId: o.voiceId,
          tags: o.tag,
          callbackUrl: o.callbackUrl,
          externalTaskId: o.externalTaskId,
  ...
  elements
    .command('get <taskId>')
    .description('one creation task (the element id is in its output)')
    .action(
      handler1<string, object>(async (taskId, _o, cmd) => {
        const g = globalsOf(cmd);
        printTasks(g, [await makeClient(g).elements.get(taskId)]);
      })
    );
  page(elements.command('list').description('your custom elements')).action(
    handler<PageOpts>(async (o, cmd) => {
      const g = globalsOf(cmd);
      printTasks(g, await makeClient(g).elements.list({ pageNum: o.page, pageSize: o.pageSize }));
    })
  );
  page(elements.command('presets').description('the official element library')).action(
    handler<PageOpts>(async (o, cmd) => {
      const g = globalsOf(cmd);
      printTasks(
        g,
        await makeClient(g).elements.presets({ pageNum: o.page, pageSize: o.pageSize })
      );
    })
  );
  elements
    ...
    .option(
      '--kind <video|image>',
      'which of the two vendor delete paths to call (same store)',
      'video'
    )
    .action(
      handler1<string, ElementDeleteOptions>(async (elementId, o, cmd) => {
        const g = globalsOf(cmd);
        const r = await makeClient(g).elements.delete(elementId, { kind: o.kind });
        print(
          g,
          `deleted element ${elementId}${r.requestId ? ` (request ${r.requestId})` : ''}`,
          r
        );
      })
    );
  ...
  voices
    ...
    .option('--external-task-id <id>', 'your own id')
    .action(
      handler<VoiceCreateOptions>(async (o, cmd) => {
        const g = globalsOf(cmd);
        const client = makeClient(g);
        await finishCreate(
          client,
  ...
  page(voices.command('list').description('your custom voices')).action(
    handler<PageOpts>(async (o, cmd) => {
      const g = globalsOf(cmd);
  ...
  page(
    voices.command('presets').description('the official voice library (not the TTS catalogue)')
  ).action(
    handler<PageOpts>(async (o, cmd) => {
      const g = globalsOf(cmd);
  ...
  voices
    ...
    .description('delete a custom voice by VOICE id')
    .action(
      handler1<string, object>(async (voiceId, _o, cmd) => {
        const g = globalsOf(cmd);
  ...
  const avatar = program
    .command('avatar')
    .description('talking-head video from an image and a sound');
  avatar
    .command('create')
    ...
    .option('--external-task-id <id>', 'your own id')
    .action(
      handler<AvatarOptions>(async (o, cmd) => {
        const g = globalsOf(cmd);
        const client = makeClient(g);
        const handle = await client.avatar.create({
          image: mediaArg(o.image),
          audioId: o.audioId,
          soundFile: o.soundFile ? mediaArg(o.soundFile) : undefined,
          prompt: o.prompt,
          mode: o.mode,
          callbackUrl: o.callbackUrl,
          externalTaskId: o.externalTaskId,
        });
        await finishCreate(client, handle, g, {
          wait: o.wait,
          download: o.download,
  ...
  audio
    ...
    .requiredOption(
      '--voice-id <id>',
      "from the vendor's TTS Voice Guide (e.g. oversea_male1) — not a presets-voices id"
    )
    .requiredOption('--voice-language <zh|en>', 'language of the voice')
    .option('--voice-speed <0.8-2.0>', 'speech rate', num('voiceSpeed'))
    .action(
      handler<TtsOptions>(async (o, cmd) => {
        const g = globalsOf(cmd);
        const outputs = await makeClient(g).audio.tts({
          text: o.text,
          voiceId: o.voiceId,
        ...
        print(
          ...
          outputs
            .map((a) =>
              a.type === 'audio'
                ? `audio ${a.id}${a.mp3DurationSeconds ? ` ${a.mp3DurationSeconds} s` : ''} ${a.mp3Url ?? ''}`
                : a.type
            )
            .join('\n'),
          { outputs }
