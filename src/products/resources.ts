 ...
 */
import { buildAvatarCreate, buildElementCreate, buildTts, buildVoiceCreate, parseCreate, parseList, parseTask, type LegacyBody } from '../codecs/legacy.js';
import type { AvatarCreateParams, ElementCreateParams, ElementDeleteOptions, MediaSource, TtsParams, VoiceCreateParams } from '../codecs/params.js';
import type { PageOptions, Product, RequestOptions, Task, TaskHandle, TaskOutput } from '../codecs/task.js';
import { validateAvatarCreate, validateElementCreate, validatePage, validateTts, validateVoiceCreate } from '../config/validators/resources.js';
...
abstract class LegacyResourceApi {
  ...
  protected async submit(product: Product, body: LegacyBody, externalId: string | undefined, signal: AbortSignal | undefined, record: Record<string, unknown>): Promise<TaskHandle> {
  ...
  protected async listAt(path: string, product: Product, options: PageOptions, maxPageSize: number): Promise<Task[]> {
    validatePage(options.pageNum, options.pageSize, maxPageSize);
    const res = await this.core.request({ method: 'GET', path, query: { pageNum: options.pageNum, pageSize: options.pageSize }, kind: 'read', signal: options.signal });
  ...
  protected async remove(path: string, body: LegacyBody, product: Product, signal: AbortSignal | undefined): Promise<DeleteResult> {
    ...
    if (typeof data === 'object' && data !== null && typeof (data as { task_id?: unknown }).task_id === 'string') {
      const task = parseCreate(res.envelope, { product, warn: this.warn });
      out.taskId = task.id;
...
export class ElementsApi extends LegacyResourceApi {
  ...
  async create(params: ElementCreateParams): Promise<TaskHandle> {
    ...
    const image = (src: MediaSource, field: string) => resolveMediaSource(src, { kind: 'image', standard: 'legacy', field, budget });
    const frontal = params.frontalImage === undefined ? undefined : image(params.frontalImage, 'frontalImage');
    const refers = params.referImages?.map((src, i) => image(src, `referImages[${i}]`));
    const videos = params.referVideos?.map((v, i) => resolveMediaSource(v, { kind: 'video', standard: 'legacy', field: `referVideos[${i}]` }));
    const externalId = resolveExternalId(params.externalTaskId);
    const body = buildElementCreate(params, { frontalImage: frontal && asField(frontal), referImages: refers?.map(asField), referVideos: videos?.map(asField) }, externalId);
  ...
  delete(elementId: string, options: ElementDeleteOptions = {}): Promise<DeleteResult> {
    const path = options.kind === 'image' ? RESOURCE_PATHS.elementDeleteImage : RESOURCE_PATHS.elementDeleteVideo;
...
export class VoicesApi extends LegacyResourceApi {
  ...
  async create(params: VoiceCreateParams): Promise<TaskHandle> {
    ...
    const externalId = resolveExternalId(params.externalTaskId);
    return this.submit('voice', buildVoiceCreate(params, externalId), externalId, params.signal, recordOf({ ...params }));
  }
...
export class AvatarApi extends LegacyResourceApi {
  ...
  async create(params: AvatarCreateParams): Promise<TaskHandle> {
    ...
    const budget = new MediaBudget();
    const image = resolveMediaSource(params.image, { kind: 'image', standard: 'legacy', field: 'image', budget });
    const sound = params.soundFile === undefined ? undefined : resolveMediaSource(params.soundFile, { kind: 'audio', standard: 'legacy', field: 'soundFile', budget });
    const externalId = resolveExternalId(params.externalTaskId);
    const body = buildAvatarCreate(params, { image: asField(image), soundFile: sound && asField(sound) }, externalId);
    const record = recordOf({ ...params, image: redactMedia(params.image, image), soundFile: sound && redactMedia(params.soundFile!, sound) });
...
export class AudioApi extends LegacyResourceApi {
  ...
  async tts(params: TtsParams): Promise<TaskOutput[]> {
    validateTts(params);
    const res = await this.core.request({ method: 'POST', path: RESOURCE_PATHS.tts, body: buildTts(params), kind: 'write', signal: params.signal });
