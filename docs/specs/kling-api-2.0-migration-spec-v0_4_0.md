...
| | |
|---|---|
| **Version** | v0.4.0 (draft — revised after pre-implementation runs #1 and #2; resubmitted) |
| **Date** | 2026-09-20 |
...
| **Review record** | Tracker project `kling-api`, pre-implementation runs (2026-09-20): **#1** architect 65 REVISE · excavator 80 · docs 69 (49 findings); **#2** on v0.3.0: architect 79 REVISE · excavator 83 · docs 83 DOCUMENTED (39 findings). §13 maps every finding of both runs to where this revision answers it. |
...
| `src/utils/polling.ts`, `src/utils/media.ts`, `src/utils/downloads.ts` | deleted (replaced by `handlers/poller.ts`, `media/*`) | 2a₀ |
| `src/utils/{logger,security,file-io,constants,index}.ts` | kept; `security.ts` hardened in 2c (D12) | — |
...
- The saver's metadata sidecar records `product`, `standard`, the *request* the handle was created from (model included) **with media fields redacted to `{ kind, bytes, sha256 }`** (a `Buffer` first frame would otherwise be JSON-stringified at ~4× its size — run #2 F7), and `raw`. `model` is not on `Task` because no vendor envelope carries it.
...
Two escape hatches for **known** ids, because the vendor also adds capabilities to existing models (run #2 A19, A26): `KlingClient({ capabilityValidation: 'warn' })` turns every capability-rule failure (resolution/duration/audio/content-type sets and the cross-field rules below) into a logger warning and sends the request anyway — shape rules (required fields, types) still throw; and every params object accepts `extraSettings?: Record<string, unknown>`, merged into `settings` (new standard) or the top-level body (legacy) *after* validation, for fields the library does not know yet. Both are documented as "the vendor moved before we did" tools, not as normal use.
...
class KlingAPIError extends KlingError { code: number; httpStatus: number; request: { kind: 'read' | 'write'; method: string; path: string }; isTransient(): boolean; isRetryable(): boolean }
                                                                                                      // isTransient(): the vendor's own signal (1302/1303/5000-5002, 429/502/503/504)
                                                                                                      // isRetryable(): isTransient() AND request.kind === 'read' — ALWAYS false for a create/delete (run #2 A17)
class KlingNetworkError extends KlingError { cause: unknown }                                         // fetch TypeError: DNS, refused, reset — no response received
class KlingTimeoutError extends KlingError { deadlineMs: number; attempt: number; attempts: number }  // our AbortController fired (covers headers AND body read); worst case for a read ≈ attempts × deadlineMs + backoff (~93 s at defaults — documented in README)
...
class KlingBatchError extends KlingError { tasks: Task[]; missing: string[]; cause: unknown }          // tasks.get: a chunk failed after earlier chunks succeeded
...
A `1303` on a create surfaces immediately as `KlingAPIError` with `isTransient() === true` and `isRetryable() === false`; the consumer decides whether to wait for a slot and re-submit — the library does not hide queue pressure (A13), and it does not hand out a method that says "retryable" for the request class it refuses to retry (run #2 A17). `POST /tasks` is a query despite the verb and is classed as a read.

**Lost-response creates are recoverable.** Every create carries an `external_task_id` — the caller's, or a UUID the library generates — and `TaskHandle.externalId` is always set. A create that ends in `KlingTimeoutError` or a post-request `KlingNetworkError` throws with `externalId` attached (`error.externalId`); the consumer recovers with `tasks.get([externalId], { byExternalId: true })` instead of re-submitting (run #2 A20). Whether the vendor *rejects* a duplicate `external_task_id` is §11 Q11.
...
| `handlers/poller.ts` | `http/errors` |
| `webhooks.ts` | `codecs/*`, `http/errors`, `node:crypto` |
| `config/loaders.ts`, `config/models.ts`, `config/constants.ts` | `codecs/task` (types) only |
| `utils/*` | `config/constants`, `node:*` only |
| `client.ts` | `products/*`, `handlers/saver`, `http/core`, `config/*` |
| `cli/*` | `client`, `index`, `handlers/*`, `ora`, `dotenv`, `commander` |

No module imports `client.ts` or `cli/*` except `index.ts` / `cli/index.ts`. Enforcement: `madge --circular src` and `eslint-plugin-import`'s `no-restricted-paths` with this table transcribed into `eslint.config.js`, both added in 1a with a failing control (D20).
...
11. **Retry classification** — mocked `1303` then `200` on a *read* → one retry after ≥1 s; mocked `1303` on a *create* → thrown immediately, `isTransient() === true`, **`isRetryable() === false`**, fetch called once; mocked network error mid-body on a create → thrown with `externalId`, fetch called once; mocked `ECONNREFUSED` on a create → retried.
...
13. **Webhook** — vendor test vector passes; flipped body byte → `KlingWebhookError('bad-signature')`; skew 301 s → `'stale-timestamp'`; no headers with a secret → `'missing-headers'`; no secret → `verified: null`.
14. **Real-undici integration** — three tests against a local `http.createServer`, Node 20 and 22: manual-redirect `Location` exposure; `cause.code` on refused connection; abort mid-body.
...
|---|---|---|---|
| **0 — Gate** (`main`) | Pricing fetched (done); README notice at top (done); `docs/api/README.md` counts (done); CHANGELOG `[Unreleased]` (done); **two paid legacy write probes with the API Key** — `POST /v1/images/generations` (`kling-v3`, `n=1`, 1k) and `POST /v1/audio/tts` (0.05 units) — two product families, since `1103` is per-resource (run #2 A18); remove `release.yml`/`.releaserc.json`/semantic-release deps; extend `ci.yml` (push + PR on `main`, `release/**`). | ~60 | Both probes `code: 0`; `npm ci && npm test` green on `main`; spec + checklist at v0.4.0 committed. |
| **1a — Transport** (additive) | `http/errors.ts`, `http/core.ts` (deadline, retry policy D10, JSON/non-JSON/3xx handling), vendor error table in `config/constants.ts` (added beside the old), `loadApiKey`, `client.ts` (`KlingClient` exposing `http` only) + `index.ts` (client + errors), `madge` + `eslint-plugin-import` + resolver + `eslint.config.js` rules + illegal-import control, `test/2.0/` tree. **Nothing deleted; 1.x still builds and its tests still run.** | ~450 src+test | V4 (new files only), V11, V12; smoke V5 via `client.http.request()` raw calls. |
| **1b — Task model + codecs** | `codecs/task.ts`, `codecs/new-standard.ts` (parsers), `codecs/legacy.ts` (parsers), fixtures for every *Response Example* in scope (fixture JSON outside the budget; files listed). | ~480 | V1 (parse half), V2. |
| **2a₀ — Remove 1.x** | The §3.1 deletion commit: `auth.ts`, `client/`, `api.ts`, `operations/`, `types.ts`, `errors.ts`, old handlers/utils, `cli.ts`, 16 tests; stub `cli/index.ts`; `package.json` `main`/`bin`/`exports` switched; `axios`, `jsonwebtoken`, `@types/jsonwebtoken`, `nock` removed. | deletion only | `npm run build && npm test` green with only `test/2.0/`; `npm pack --dry-run` shows the 2.0 layout. |
| **2a₁ — Tasks + poller** | `products/tasks.ts` (`createHandle`, `get` → `{tasks, missing}`, `list`, `getByProduct`, `listByProduct`, `handle`, `healthCheck`), `handlers/poller.ts`, UUID `external_task_id` generation, `KlingBatchError`. | ~350 | Handle concurrency test; chunking + `missing` + `KlingBatchError` tests; live `tasks.list({limit:1})` closes Q2; live 100-id `get` closes Q13. |
| **2a₂ — Video t2v/i2v** | `VIDEO_MODELS`, new-standard builders for t2v/i2v, `products/video.ts` (two methods), validators for t2v/i2v with `capabilityValidation` mode and `extraSettings`. | ~500 | V1 (8 fixtures), V3; V6 opt-in; Q11 probe. |
| **2b — Video omni + motion** | builders + validators for omni and motion-control, count matrices, `@`-reference check. | ~420 | V1, V3; V10 omni opt-in (not release-blocking). |
| **2c — Media + save** | `media/source.ts` (per-standard caps), `media/download.ts` (D12, hardened `validateUrl`), `handlers/saver.ts` (D14, `client.save`, `force`, sidecar redaction). | ~450 | Byte-cap, redirect-cap, per-hop SSRF (IPv4 alt-encoding + IPv6 + DNS) tests each with a failing control; `save()` on the V6 task writes an mp4. |
| **3a — Image generate + omni** | `IMAGE_MODELS`, legacy builders, `products/image.ts` (generate, omni), validators. | ~420 | V1, V3; V10 image (release-blocking; the Phase 0 probe task is the first evidence). |
...
| **4a — Elements + voices** | §6.3 types (now written), `products/elements.ts`, `products/voices.ts`, builders, `delete(kind)`. | ~450 | Contract tests; live `presets()` both ≥ 1; Q7 probe. |
| **4b — Avatar + TTS** | `products/avatar.ts` (port), `products/audio.ts`. | ~250 | Contract tests; TTS live call (already exercised in Phase 0). |
| **5 — Platform** | `products/account.ts` (usage, ledgers), `webhooks.ts` (`verifyWebhookSignature`, `parseCallback` with `KlingWebhookError`). | ~350 | V13; Q1 if a receiver is available (not release-blocking). |
| **6a₁ — CLI core + video** | `cli/index.ts` (program, `.env` chain, spinner), `cli/video.ts`. | ~450 | `kling --help` V8 checks; video subcommand tests. |
| **6a₂ — CLI image + tests** | `cli/image.ts`, `test/2.0/cli.test.ts` rewritten. | ~400 | CLI tests green. |
...
2a₂/2b/2c and 3a/3b can proceed in parallel branches off `release/2.0` once 2a₁ lands (2a₀ must land first — it is the only commit that changes `package.json`'s entry points); 4a depends on 2a₁ (handles). Budget rule: fixture JSON is outside the number; an overrun > 20 % splits the sub-phase before merge (D20).
...
| 10.14 | Auto-generated `external_task_id` on every create (D10, run #2 A20) | Yes — UUID when the caller supplies none | Recommendation — proceeding unless overruled. |
...
| v0.3.0 | 2026-09-20 | Revised on run #1's 49 findings (§13.1). New: D10 retry policy and error family, D12 `MediaSource` union and download port, D19 manual release + branch strategy (semantic-release removed — Alex), D20 test seam, §5 import graph, §6 complete types and export set, §9 sub-phases with LOC budgets and a Phase 0 write-probe gate, D9 unknown-model passthrough, D4 `standard`/`product?`/`outputsExpireAt`, D5 product-neutral query names. Corrected counts (§2.5, §3), survivors (§0), D17. |
| v0.4.0 | 2026-09-20 | Revised on run #2's 39 findings (§13.2). New: §3.1 1.x disposition table and the 2a₀ removal commit (1a is additive); 2a/6a split; write-aware `isRetryable()` + `isTransient()`; auto `external_task_id`; `parseCallback` throws on failed verification; per-standard media caps (10 MB legacy); hardened `validateUrl`; `redirect: 'manual'`; `tasks.get` → `{ tasks, missing }` + `KlingBatchError`; `capabilityValidation`/`extraSettings`; `client.save`/`force`; sidecar redaction; `webhooks.ts`; §6.3 written; import-lint control; real-undici tests; V14/V15; Q14. Corrected ERROR_CODES to 22 rows. |
