...
**Status 2026-09-20:** docs + package landed in `6e983cc`; 419 tests; V5/V7/V8/V9 green; docs snapshot verified current against today's `llms.txt` (26 pages differ only by a leading BOM / a `> Current Tab` header line, zero contract changes; nine new out-of-scope pages: e-commerce, protocols, pricing solutions). **Remaining: merge to `main`, `npm publish` (Alex), consumer install.**

### Docs
- [x] `README.md` rewritten: install; `KLING_API_KEY`; one example per namespace; `TaskHandle`/`Task` usage; `MediaSource` rules (string is URL/Base64, `{ path }` for files); model tables generated from `VIDEO_MODELS`/`IMAGE_MODELS`; **default-model policy stated** (video: cheapest current-gen *with native audio*; image: newest — D6) with the dated price table; retry semantics (**`taskState`** drives re-submit decisions; `isTransient` is diagnostic; creates never auto-retried; `tasks.recover(product, externalId)` with `null` = not visible; TTS unrecoverable; body-size-scaled create timeout); worst-case read time arithmetic (D11); fetch parity notes (no env proxy; `fetch` injection); webhook raw-body precondition and `verified: null` warning; migration pointer to spec §7; ToC with unique headings; no empty headings
- [x] V9: the literal grep is not achievable as written — `task_id` matches `external_task_id` / `task_ids` (the vendor's own field names, which the README must name) and the migration table's 1.x column carries `accessKey`, `kling-v1`, `'succeed'` by design. **Refined and enforced in `test/2.0/readme.test.ts`:** outside the migration table, `accessKey|secretKey|kling-v1\b|kling-v2-master|kling-v2-6|kling-video-o1|'succeed'|KlingAPI` → 0; bare `task_id` → exactly 1 (the legacy callback body key); `kling-image-o1` → 1 (the model table)
- [x] `CHANGELOG.md`: `## [Unreleased]` → `## [2.0.0] - <date>`; *Removed* / *Changed* / *Added* transcribed from spec §7; *Changed* names the semantics-without-signature items: `TaskStatus` `succeed → succeeded`, default models + policy, string `MediaSource` no longer a path, `outputsExpireAt`
- [x] `docs/api/README.md` refresh one-liner run into a temp dir (59 pages today vs 50 committed); matched by `> Source:` URL: 24 identical, 26 differ only by a UTF-8 BOM at byte 0 (the committed copies are BOM-stripped) plus a `> Current Tab:` header line on `kling-img-recog.md` / `kling-multi-element-video-editing.md` (out of scope) — **zero contract changes**; the 9 new pages are e-commerce replication (4), protocols (3), pricing solutions (2), all out of scope

### Package
- [x] `package.json`: `version: "2.0.0"`; `engines.node >=20.0.0`; `dependencies` = `commander`, `dotenv`, `ora`; devDependencies has `madge`, `eslint-plugin-import-x` (the maintained fork — 1c), `eslint-import-resolver-typescript`; no `semantic-release`/`nock`/`axios`/`jsonwebtoken`; `npm run verify` added (lint → cycles → build → test → tsc → dist layout)
- [x] `ci.yml`: package-layout step — `dist/index.js` + `dist/cli/index.js` present, `dist/api.js` / `dist/auth.js` / `dist/cli.js` absent, `npm pack --dry-run` lists `dist/index.js`
...
- [x] **(V7)** → 0 on `src` (2026-09-20) `grep -rnE "kling-v1\b|kling-v1-5|kling-v1-6|kling-v2-master|kling-v2-1-master|kling-v2-5-turbo|kling-v2-6|kling-v2-new|kling-video-o1|'kling-v2'" src` → 0
- [x] **(V8)** `npm pack --dry-run`: `dist/index.js`, `dist/cli/index.js`, 168 files, 164.5 kB; no `dist/auth.*` / `dist/api.js` / `dist/cli.js`; the barrel prints every §6.4 value (KlingClient, the 15 error classes + `KlingTaskNotFoundError`, `verifyWebhookSignature`, `parseCallback`, `save`, `ERROR_CODES`, `VIDEO_MODELS`, `IMAGE_MODELS`, `BASE_URL`) plus the product classes and helpers
- [x] **(V5)** `scripts/smoke.mjs` against the 2.0.0 `dist/`: 4/4 live reads 200, garbage-key control 401/1002 (2026-09-20)
- [x] **(V15) Release-blocking blanks filled** in this file (all ticked above: Phase 0 five probes, 1c smoke, Q2, Q13, V6 + audio, Q11, V10 image, Q15 for image-generation and voice; not blocking and still open: Q1, Q9-revoked, Q15 for the other legacy products): Phase 0 all five probes, 1c smoke `code`, 2a₁ Q2 and Q13, 2a₂ V6 (incl. audio-track check) and Q11, 3a V10 image, Q15 per legacy product. (Not blocking: 2b V10 omni, 5 Q1, Q9-revoked. The three Phase 0 resource probes ARE blocking — §10.15.)
...
| Q11 `external_task_id` idempotency | Phase 2a | **closed 2026-09-22 [LIVE, Phase 7] — scope now probed, and it is WIDER than feared** — duplicate → 400 / `1201` "External_task_id … already exists"; not-created, no charge. Phase 7 probe (`scripts/phase7-dedupe.mjs`, 2026-09-22) refused the duplicate with `1201 / not-created` in every scope tested: immediately AND ~13 min later; on a new-standard create (`text-to-video`) AND a legacy one (`image.generate`); and ACROSS products — a video's id re-used on an image create is refused, so the namespace is account-wide, not per-product. A refused create costs nothing (seven `1303` refusals in a separate run moved the balance by 0.00, verified against `packageLedger`). Still unobserved: a re-submit after a **failed** original — deliberately not forced, since it means either tripping moderation or paying for a task designed to break. |
