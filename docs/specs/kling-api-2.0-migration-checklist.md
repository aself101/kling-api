...
| Q11 `external_task_id` idempotency | Phase 2a | **closed 2026-09-20 [LIVE], n=1 — scope narrower than the wording suggested** — duplicate → 400 / `1201` "External_task_id … already exists"; not-created, no charge. Observed **once**, on a `text-to-video` create re-submitted immediately after a *successful* one. NOT observed: on legacy creates, on other products, after a *failed* or timed-out original, or after any elapsed time. The README hedges it correctly; this row said "It IS an idempotency key" flatly until 2026-09-22, and the double-bill-corridor guidance rests on it — so the remaining scope is a Phase 7 probe, not a closed question. |
