...
8. **Naming drift: "omni" files documenting `kling-3.0`** — `kling-omni-3.0-t2v.md`, `kling-omni-3.0-i2v.md`, `kling-omni-3.0-motion-control.md` all target path model `kling-3.0`, not `kling-3.0-omni`; only `kling-omni-3.0-ovg.md` targets `kling-3.0-omni`. The vendor groups them under the "3-0-omni" nav tab. Also the capability map lists Motion Control as "Not Supported" for Kling 3.0 Omni but "Supported" for Kling 3.0 — consistent with the path, inconsistent with the file names. (The file was fetched with a stray " copy" suffix and renamed to `kling-omni-3.0-t2v.md` before the snapshot was committed.)
