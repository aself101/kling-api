...
> Source: https://kling.ai/document-api/api/video/2-6/text-to-video
> Locale: en
...
- `settings.audio`: When generating videos with native audio, only 1080P resolution is supported.
...
```bash
curl --location 'https://api-singapore.klingai.com/text-to-video/kling-2.6' \
