...
> Source: https://kling.ai/document-api/api/video/2-5-turbo/image-to-video
> Locale: en
...
- Method: `POST`
- Path: `/image-to-video/kling-2.5-turbo`
...
|---|---|---:|---|---|---|
| `contents` | array | Yes | - | - | Reference input collection (prompts, images, etc.) |
| `contents[].type` | string | Yes | - | `prompt`, `first_frame`, `last_frame` | Input type. Supports: prompt, first frame, last frame |
...
- `contents[].type.prompt`: ```json
  {
    ...
    "text": "string" // The content of the text prompt should not exceed 2500 characters in length; required
  }
  ```
...
- `contents[].type.first_frame / last_frame`: Supports first-frame-to-video and first-and-last-frame-to-video generation; last-frame-only video generation is not supported.
    - First + last frame only supports 1080p resolution.
...
- `settings.resolution`: When using the first and last frame function, only 1080P video generation is supported.
- `options`: ```json
  "options": {
...
```bash
curl --location 'https://api-singapore.klingai.com/image-to-video/kling-2.5-turbo' \
