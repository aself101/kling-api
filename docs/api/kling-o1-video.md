...
> Source: https://kling.ai/document-api/api/video/o1/video-omni
> Locale: en
> Current Tab: Video Generation
> Sibling Tabs: Video Generation / Element Management
...
| `settings.duration` | int | No | `5` | `3`, `4`, `5`, `6`, `7`, `8`, `9`, `10` | Video duration in seconds |
...
- `contents[].type.prompt`: Defined in JSON format, as follows:
  ...
  ```json
  {
    ...
    "text": "string" // Text prompt, which can include positive and negative descriptions, with a content not exceeding 2500 characters; required
  }
  ```
  
  - The prompt can be templated to meet different video generation requirements:
- `contents[].type.prompt`: > **The Kling O1 model can achieve various capabilities through prompt and other content.**
  >
  > 
  > 1. Specify an image, an Element, a video in the format of @xxx, such as @image_1, @Zhang, @video_1.
  >
  > 
  > 2. Avoid Element names that are substrings of each other, such as @Zhang and @ZhangSan.
  >
  > 
  > 3. Avoid using Element names that overlap with content in the prompt. For example, do not use @gmail as an Element name if the prompt contains "My email address is xxx@gmail.com".
  >
  > 
  > 4. For more information, please refer to: [Kling O1 Model User Guide](https://kling.ai/quickstart/klingai-video-o1-user-guide).
- `contents[].type.first_frame / last_frame / refer_image`: Images can be used as reference images for scenes, styles, etc., and can also be used as the first or last frame to generate videos.
  - Defined in JSON format, as follows:
  ...
  - The width and height dimensions of the image should not be less than 300px, and the aspect ratio of the image should be between 1:2.5 and 2.5:1.
  - Image quantity limits depend on the number and type of reference elements:
    - No reference video + multi-image elements: the total number of reference images and multi-image elements must not exceed 7.
    - With a reference video + multi-image elements: the total number of reference images and multi-image elements must not exceed 4.
  - When an image is used as the first frame or last frame:
    - Only "first frame only" and "first + last frame" are supported. "Last frame only" is not supported yet.
    - When using both the first and last frames, no additional reference images can be added.
- `contents[].type.feature_video / base_video`: Defined in JSON format, as follows:
  
  ```json
  {
    "type": "string", // Video identifier. Must match enum values: feature_video, base_video; required
  ...
  - The video file size cannot exceed 200MB.
  - The video duration should be between 3 seconds (inclusive) and 10 seconds (inclusive).
  - The width and height dimensions of the video should be between 700px (inclusive) and 2160px (inclusive).
  - The video frame rate is based on 24fps to 60fps (the frame rate for generating videos is 24fps).
  - Add at most one reference video.
  - When using a video as a feature reference video:
    - Only supports defining the first frame of the video, not the last frame.
  - When a base video is used as a video to be edited:
    - Does not support defining the first or last frame of a video.
- `contents[].type.element`: Defined in JSON format, as follows:
  ...
  - Elements fall into two categories: video elements (customized from video) and multi-image elements (customized from multiple images). Currently, the model only supports multi-image elements; video elements are not yet supported.
  - The upper limit of the number of element is related to the type of element, the number of reference pictures, the presence or absence of reference videos and other factors, among which:
...
### First_frame & multi-image_elements & feature_video
