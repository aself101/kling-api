...
> Source: https://kling.ai/document-api/api/video/2-6/image-to-video
> Locale: en
...
|---|---|---:|---|---|---|
| `contents` | array | Yes | - | - | Reference input collection (prompts, images, voices, etc.) |
| `contents[].type` | string | Yes | - | `prompt`, `first_frame`, `last_frame`, `voice` | Input type. Supports: prompt, first frame, last frame, voice |
...
- `contents[].type.prompt`: ```json
  ...
  ```
- `contents[].type.prompt`: Specify a voice in the format of @xxx, such as @sweet.
...
- `contents[].type.first_frame / last_frame`: Supports first-frame-to-video and first-and-last-frame-to-video generation; last-frame-only video generation is not supported. Among which:
    - When generating videos using the first and last frames, only 1080p resolution is supported.
- `contents[].type.voice`: Defined in JSON format, as follows:
- `contents[].type.voice`: ```json
  {
    "type": "voice", // Input type, fixed parameter value: voice; required
    "voice_id": "string", // Voice ID, generated by the system, is returned through custom voice API; required
  ...
  ```
- `contents[].type.voice`: At most 2 voices can be referenced.
- `contents[].type.voice`: When specifying a voice, the sound parameter audio cannot be off.
- `contents[].type.voice`: The voice_id is returned by the Custom Voice API, or you can use a system preset voice. [Click here for details](https://kling.ai/document-api/api/video/2-6/voice-customization).
...
- `settings.audio`: When audio=native, only 1080P videos are supported.
- `settings.audio`: When audio=off, designated voice is not supported.
...
- `settings.resolution`: When generating videos with native audio, only 1080p resolution is supported.
- `settings.resolution`: When generating videos using the first and last frames, only 1080P videos can be generated.
- `options`: ```json
  "options": {
...
```bash
curl --location 'https://api-singapore.klingai.com/image-to-video/kling-2.6' \
