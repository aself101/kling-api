...
> Source: https://kling.ai/document-api/api/video/2-6/motion-control
> Locale: en
> Current Tab: Motion Control
> Sibling Tabs: Text to Video / Image to Video / Motion Control / Voice Management
...
|---|---|---:|---|---|---|
| `contents` | array | Yes | - | - | Reference input collection (prompt, appearance reference image, motion reference video) |
| `contents[].type` | string | Yes | - | `prompt`, `image`, `video` | Input type. Supports: prompt, appearance reference image, motion reference video |
...
- `contents`: ```json
  ...
  ```
- `contents[].type`: `prompt`: prompt input identifier.
  - `image`: appearance reference image input identifier.
  - `video`: motion reference video input identifier.
...
- `contents[].type.image`: ```json
  {
    "type": "image", // Reference image input identification, fixed parameter value: image; required
...
```bash
curl --location 'https://api-singapore.klingai.com/motion-control/kling-2.6' \
